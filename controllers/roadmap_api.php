 ...
 * Serves level data in batches for infinite scroll functionality
 */

// Set JSON response headers
header('Content-Type: application/json');

// Enable CORS if needed (optional, for development)
...
try {
    // Fetch levels for this course with pagination
    $levels = LevelModel::getLevelsByCourseWithPagination($pdo, $courseId, $limit, $offset);
    
    // Get total count for this course
    $totalLevels = LevelModel::countLevelsByCourse($pdo, $courseId);
    ...
    $baseTop = 200; // Base vertical position
    ...
    foreach ($levels as $index => $level) {
        ...
        $status = determineNodeStatus($pdo, $userId, $levelId, $level['parent_level_id'], $completedLevels);
        
        // Calculate position
        // For absolute positioning across all pages, use order_in_course
    ...
    $response = [
        ...
        'has_more' => ($offset + count($levels)) < $totalLevels,
        'levels' => $roadmapData
    ];
    ...
    error_log("Roadmap API Error: " . $e->getMessage());
 ...
 */
function determineNodeStatus(PDO $pdo, int $userId, int $levelId, ?int $parentLevelId, array $completedLevels): string
{
    // DEVELOPMENT MODE: Temporarily unlock all levels for testing
    if (DEVELOPMENT_MODE) {
        // Check if completed
        if (in_array($levelId, $completedLevels)) {
            return 'completed';
        }
        // All non-completed levels are 'current' (unlocked) in dev mode
        return 'current';
    }
    
    // PRODUCTION LOGIC (currently inactive):
